...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({
  ...
  const handleSignIn = () => {
    if (!agreed) return;
    // Hardcoded login — replace with Supabase OAuth when keys are ready
    onSuccess({
      ...
      <div className="fixed inset-0 z-[9001] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
        <div className="pointer-events-auto w-full max-w-3xl relative overflow-hidden rounded-3xl border border-white/10 bg-[#0f0f13] text-white flex flex-col md:flex-row shadow-2xl shadow-black/80 animate-[scaleIn_0.25s_ease-out]">
          ...
          <div className="w-full md:w-1/2 relative h-56 md:h-auto min-h-[320px] overflow-hidden border-b md:border-b-0 md:border-r border-white/10 bg-black">
            <img
              src="https://res.cloudinary.com/dgd5sfnrq/image/upload/v1771391868/leftSideImage_eekf5p.png"
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center items-center text-center bg-white dark:bg-[#111116]">
            ...
            <div className="flex items-start space-x-3 mb-6 text-left max-w-[300px] w-full">
              ...
              />
              <label htmlFor="terms" className="text-xs font-medium text-slate-500 dark:text-white/60 cursor-pointer leading-relaxed">
                I agree to the{' '}
                <a href="https://drive.google.com/file/d/1csFflUnzzIWuoddthbWJbvbDFn78-4dP/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-slate-800 dark:text-white/90 hover:underline font-semibold">Terms</a>
                {' '}and{' '}
                <a href="https://drive.google.com/file/d/1ZvXVMSF_y3kL4_hd5ciOk3zM9KR3ZFhu/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-slate-800 dark:text-white/90 hover:underline font-semibold">Privacy Policy</a>
              </label>
            </div>

            <div className={`w-full max-w-[300px] transition-all duration-300 ${agreed ? 'opacity-100' : 'opacity-40 cursor-not-allowed'}`}>
              <button
                onClick={handleSignIn}
                disabled={!agreed}
                className={`w-full h-11 rounded-full flex items-center justify-center gap-3 px-4 text-sm font-semibold transition-all shadow-md ${agreed ? 'bg-[#202124] hover:bg-[#2c2d30] text-white hover:scale-[1.01] active:scale-95 cursor-pointer border border-white/10' : 'bg-zinc-600/50 text-zinc-400 cursor-not-allowed'}`}
              >
